...
use crate::{tabs::*, app::AppContext, theme::THEME};
...
impl Root<'_> {
    fn render_title_bar(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph::new(Span::styled("DLMM_UI", THEME.app_title)).render(area[0], buf);
        let titles = vec![" about ", " Weather ", " dashboard "];
        Tabs::new(titles)
            .style(THEME.tabs)
    ...
    fn render_selected_tab(&mut self, area: Rect, buf: &mut Buffer) {
        ...
        match self.context.tab_index {
            ...
            1 => WeatherTab::new(row_index).render(area, buf),
            2 => DashboardTab::new(&mut self.context.input).render(area, buf),
            _ => unreachable!(),
        };
